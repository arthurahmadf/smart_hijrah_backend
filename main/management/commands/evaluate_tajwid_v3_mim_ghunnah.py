...
SUPPORTED_RULE_CODES = MIM_RULE_CODES | GHUNNAH_RULE_CODES


class Command(BaseCommand):
    help = (
        "Evaluasi exact-span detector v3 Mim Sakinah dan Ghunnah "
        "Musyaddadah terhadap goldset."
    )
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--fail-on-errors",
            action="store_true",
            help="Return exit non-zero jika ada case gagal.",
        )
        parser.add_argument(
            "--output-dir",
            default="reports/tilawah/v3_mim_ghunnah",
        )
    ...
    def handle(self, *args, **options):
        ...
        summary_path = output_dir / "tajwid_v3_mim_ghunnah_summary.json"
        mismatch_path = output_dir / "tajwid_v3_mim_ghunnah_mismatches.csv"
        ...
        self.stdout.write("Tajwid v3 — Mim Sakinah/Ghunnah Evaluation")
        ...
        if options["fail_on_errors"] and not report.success:
            raise CommandError("Detector mim/ghunnah belum lulus goldset.")
