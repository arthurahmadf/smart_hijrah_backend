...
from main.utils_tilawah.tajwid_v3.detectors.ra import (
    RaDetector,
    SUPPORTED_RULE_CODES as RA_RULE_CODES,
)
...
AUDITED_RULE_CODES = RA_RULE_CODES


class Command(BaseCommand):
    help = (
        "Audit corpus read-only untuk detector Ra Tafkhim/Tarqiq v3. Tidak membuat annotation set atau mengubah level ayat."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output-dir",
            default="reports/tilawah/v3_ra_corpus",
        )
    ...
    def handle(self, *args: Any, **options: Any):
        ...
        for position, ayah in enumerate(
            ...
            try:
                result = analyze_tajwid_v3(
                    ...
                    detectors=(RaDetector(verse_key=f"{ayah.surah_number}:{ayah.ayah_number}"),),
                )
                relevant = [
        ...
        summary_path = output_dir / "tajwid_v3_ra_corpus_summary.json"
        issue_path = output_dir / "tajwid_v3_ra_corpus_issues.csv"
        ayah_path = output_dir / "tajwid_v3_ra_corpus_ayahs.csv"
        distribution_path = output_dir / "tajwid_v3_ra_distribution.csv"
