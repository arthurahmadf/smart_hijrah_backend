...
from main.utils_tilawah.tajwid_v3.detectors.advanced_mad import (
    AdvancedMadDetector,
...
class Command(BaseCommand):
    help = (
        "Audit corpus read-only untuk Advanced Mad Resolver v3. "
        "Tidak membuat annotation set dan tidak mengubah level ayat."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--reading-mode", choices=("ayah_stop", "wasl", "waqf"), default="ayah_stop"
        )
        parser.add_argument("--limit", type=int, default=None)
        parser.add_argument("--chunk-size", type=int, default=250)
        parser.add_argument("--output-dir", default="reports/tilawah/v3_advanced_mad_corpus")
    ...
    def handle(self, *args: Any, **options: Any):
        ...
        self.stdout.write(f"Tajwid v3 Advanced Mad corpus audit — Engine {ENGINE_VERSION}")
        ...
        for position, ayah in enumerate(
            queryset.iterator(chunk_size=options["chunk_size"]), start=1
        ):
            ...
            try:
                result = analyze_tajwid_v3(
                    ...
                    detectors=(AdvancedMadDetector(verse_key=verse_key),),
                    verse_key=verse_key,
                )
                relevant = [
            ...
            for annotation in relevant:
                ...
                if annotation.confidence < 1.0:
                    counters["provisional_annotations"] += 1
            ...
            ayah_rows.append({
                ...
                "provisional_count": sum(1 for item in relevant if item.confidence < 1.0),
        ...
        summary = {
            ...
            "audited_rule_codes": sorted(SUPPORTED_RULE_CODES),
            "counters": dict(sorted(counters.items())),
            "issue_type_counts": dict(sorted(issue_counts.items())),
        }
        summary_path = output_dir / "tajwid_v3_advanced_mad_corpus_summary.json"
        issue_path = output_dir / "tajwid_v3_advanced_mad_corpus_issues.csv"
        ayah_path = output_dir / "tajwid_v3_advanced_mad_corpus_ayahs.csv"
        distribution_path = output_dir / "tajwid_v3_advanced_mad_distribution.csv"
        summary_path.write_text(
            json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
        )
        self._write_csv(issue_path, issue_rows, [
            "ayah_id", "surah_number", "ayah_number", "issue_type", "severity", "detail", "evidence"
        ])
        self._write_csv(ayah_path, ayah_rows, [
            "ayah_id", "surah_number", "ayah_number", "annotation_count",
            "provisional_count", "emitted_rule_codes", "issue_count", "exception"
        ])
        distribution_rows = []
        for code in sorted(SUPPORTED_RULE_CODES):
