...
from main.utils_tilawah.tajwid_v3.detectors.orthographic_waqf import (
    ...
    SUPPORTED_RULE_CODES as AUDITED_RULE_CODES,
)
...
class Command(BaseCommand):
    help = (
        "Audit corpus read-only Stage 5L: Hamzat Wasl, silent letters, Saktah, "
        "dan metadata tanda waqaf. Tidak menulis annotation/database."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output-dir",
            default="reports/tilawah/v3_orthographic_waqf_corpus",
        )
    ...
    def handle(self, *args: Any, **options: Any):
        ...
        destination_rows = TilawahAyahPool.objects.filter(
            surah_number__in={int(key.split(":")[0]) for key in destination_keys}
        ...
        waqf_hint_counts: Counter[str] = Counter()
        ...
        for position, ayah in enumerate(
            ...
            if options["reading_mode"] == "wasl" and verse_key in boundary_map:
                boundary_to = boundary_map[verse_key]
                next_text = destination_text.get(boundary_to)
                if next_text:
                    input_text = f"{input_text} {next_text}"
                else:
            ...
            try:
                ...
                relevant = [
                    ...
                    and item.trigger_span.codepoint_start < source_text_length
                ]
                issues = [
                    item
                    for item in result.issues
                    if item.grapheme_index is None
                    or result.stream.grapheme(item.grapheme_index).codepoint_start
                    < source_text_length
                ]
                hints = [
                    item
                    for item in result.waqf_hints
                    if item.span.codepoint_start < source_text_length
                ]
            ...
            for hint in hints:
                waqf_hint_counts[hint.kind.value] += 1
                counters["total_waqf_hints"] += 1
        ...
        summary = {
            ...
            "waqf_hint_counts": dict(sorted(waqf_hint_counts.items())),
        }

        summary_path = output_dir / "tajwid_v3_orthographic_waqf_corpus_summary.json"
        issue_path = output_dir / "tajwid_v3_orthographic_waqf_corpus_issues.csv"
        ayah_path = output_dir / "tajwid_v3_orthographic_waqf_corpus_ayahs.csv"
        distribution_path = output_dir / "tajwid_v3_orthographic_waqf_distribution.csv"
