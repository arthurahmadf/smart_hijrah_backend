...
def build_feedback(ayah_text, transcript):
    """
    Gabungkan hasil word matching + tajwid jadi format response frontend.

    Args:
        ayah_text: teks ayat referensi dari database (dengan harakat)
    ...
    # Step 2 — Tajwid analysis dari teks referensi
    tajwid_result = analyze_tajwid(ayah_text)

    # Step 3 — Build feedback items
    feedback_items = []
    feedback_id = 1

    # Kelompokkan word results jadi segmen correct/correction
    ...
    for segment in segments:
        item = {
            ...
            'audio_url': None  # Skip untuk MVP
        }
        feedback_items.append(item)
        feedback_id += 1

    # Step 4 — Hitung tajwid score
...
def _build_segments(word_results, tajwid_result):
    ...
    for word_result in word_results:
        ...
        if status == 'correct':
            ...
            # Cek apakah ada tajwid rule untuk kata ini
            ...
            # Kata tambahan dari user — tidak increment word_index
            # Gabung ke correct segment terakhir kalau ada, skip saja
            pass
